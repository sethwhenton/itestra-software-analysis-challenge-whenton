...
import static cronutils.model.field.expression.FieldExpressionFactory.*;
import cronutils.builder.CronBuilder;
...
public class LogInfo {
  ...
  private static ThreadLogInfo getInfo() {
    ...
    synchronized (threadInfos) {
      for (ThreadLogInfo info : threadInfos) {
        if (info.getThread() == currentThread)
          return info;
      }
  ...
  public static void begin_threads() {
    if (threadInfos != null)
      throw new RuntimeException("Already in thread mode");
    mainInfo.flush();
    threadInfos = new ArrayList<ThreadLogInfo>();
  }
  ...
  public static void end_threads() {
    if (threadInfos == null)
      throw new RuntimeException("Not in thread mode");
    for (ThreadLogInfo info : threadInfos)
      info.flush();
    threadInfos = null;
  }
...
class ThreadLogInfo {
  ...
  private boolean buffered; // Whether we buffer everything that's printed, and only displaying it when
                            // flush() is called.
  ...
  public synchronized void end_track() {
    if (indLevel == 0) {
      indLevel = 1;
      // throw new RuntimeException("Already at indLevel = 0, can't decrement (you
      // probably have too many end_track's)");
    }
    indLevel--;
  ...
  public void error(Object o) {
    if (LogInfo.numErrors++ < LogInfo.maxPrintErrors)
      print("ERROR: " + o + "\n");
  }
  ...
  public void warning(Object o) {
    if (LogInfo.numWarnings++ < LogInfo.maxPrintWarnings)
      print("WARNING: " + o + "\n");
  }

  public void fails(String format, Object... args) {
